...
--   ProductArg (ConCol "obsolete" '[] as) "con" arg s0 =>
--   CompColumn ('Prod ('Con as)) 'Nest (ConCol "obsolete" '[] as) s0 s1 =>
--   arg ->
--   Dd s1
-- con =
--   compFor @"con" @('Prod ('Con as)) @'Nest @(ConCol "obsolete" '[] as)

-- conAs ::
--   ∀ (sel :: Symbol) (as :: [Type]) (s0 :: [DdK]) (s1 :: DdK) (arg :: Type) .
--   ProductArg (ConCol "obsolete" '[] as) "conAs" arg s0 =>
--   Rename s1 (SetName s1 sel) =>
--   CompColumn ('Prod ('Con as)) 'Nest (ConCol "obsolete" '[] as) s0 s1 =>
--   arg ->
--   Dd (SetName s1 sel)
-- conAs =
--   rename . compFor @"conAs" @('Prod ('Con as)) @'Nest @(ConCol "obsolete" '[] as)

-- con1 ::
--   ∀ (a :: Type) (s0 :: DdK) (s1 :: DdK) .
--   CompColumn ('Prod ('Con '[a])) 'Merge (ConCol "obsolete" '[] '[a]) '[s0] s1 =>
--   Dd s0 ->
--   Dd s1
-- con1 dd =
--   compColumn @('Prod ('Con '[a])) @'Merge @(ConCol "obsolete" '[] '[a]) (dd :* Nil)
