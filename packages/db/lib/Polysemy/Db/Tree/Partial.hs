...
import Polysemy.Db.Tree (TableTree, TreeImpl(..), TreePayload(..), TreePrim(..), TreeProduct(..), TreeProductElem (treeProductElem), tableTree)
...
instance TreeImpl PartialTag PartialField Type where
  type ConParam PartialTag PartialField Type =
    ()
  type ColParam PartialTag PartialField Type _ =
    ()

instance TreePrim PartialTag PartialField e a meta where
  treePrim _ =
    PartialField.Keep

instance TreePayload PartialTag PartialField Type d meta where
  treePayload _ =
    PartialTag

instance TreeProduct PartialTag PartialField Type d () where
  treeProduct _ =
    ()

instance TreeProductElem PartialTag PartialField e () () ('ColumnMeta name rep d) () where
  treeProductElem =
    undefined

partialTree' ::
  ∀ d e c .
  TableTree PartialTag PartialField e d c =>
  PartialTree c
partialTree' =
  tableTree @PartialTag @PartialField @e @d
